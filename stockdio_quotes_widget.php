...
class Widget_Stock_Quotes extends WP_Widget {
 
  public function __construct() {
      $widget_ops = array('classname' => 
		'Widget_Stock_Quotes', 
		'description' => 'Display list of stock prices, market indices, currencies or commodities.' );
      parent::__construct('Widget_Stock_Quotes', 'Stock Quotes List', $widget_ops);
  }
  ...
  function widget($args, $instance) {
   ...
   $culture= !isset($instance['culture']) ? '' : $instance['culture'];	
	...
	//echo $output;	
	echo stockdio_quotes_board_func( array( 
									'title' => $title , 
  ...
  public function form( $instance ) {
	...
	$stockdio_quotes_board_options = get_option( 'stockdio_quotes_board_options' );
     ...
     $instance = wp_parse_args( (array) $instance, array( 
		 'title' => array_key_exists('default_title',$stockdio_quotes_board_options)?$stockdio_quotes_board_options['default_title']:'',
		 'exchange' => array_key_exists('default_exchange',$stockdio_quotes_board_options)?$stockdio_quotes_board_options['default_exchange']:'',
		 'symbols' => array_key_exists('default_symbols',$stockdio_quotes_board_options)?$stockdio_quotes_board_options['default_symbols']:'',
		 'width' => array_key_exists('default_width',$stockdio_quotes_board_options)?$stockdio_quotes_board_options['default_width']:'',
		 'height' => array_key_exists('default_height',$stockdio_quotes_board_options)?$stockdio_quotes_board_options['default_height']:'',
		 'includechart' => array_key_exists('default_includeChart',$stockdio_quotes_board_options)?$stockdio_quotes_board_options['default_includeChart']:'',
		 'logomaxwidth' => array_key_exists('default_logoMaxWidth',$stockdio_quotes_board_options)?$stockdio_quotes_board_options['default_logoMaxWidth']:'',
		 'logomaxheight' => array_key_exists('default_logoMaxHeight',$stockdio_quotes_board_options)?$stockdio_quotes_board_options['default_logoMaxHeight']:'',
		 'culture' => array_key_exists('default_culture',$stockdio_quotes_board_options)?$stockdio_quotes_board_options['default_culture']:''
...
add_action('admin_print_styles', 'stockdio_quotes_widget_admin_styles');

function stockdio_quotes_widget_admin_styles() {
  echo '<style>
	#available-widgets-list [class*=widget_stock_quotes] .widget-title:before{
	  content: "\61";
...
//add_action( 'widgets_init', create_function('', 'return register_widget("Widget_Stock_Quotes");') );
add_action( 'widgets_init', function() {
	return register_widget("Widget_Stock_Quotes");
});
